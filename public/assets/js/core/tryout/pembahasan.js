jQuery(document).on('click', '.pembahasan-tryout', function() {
    let id = jQuery(this).data('id')
    let num = jQuery(this).data('num')
    var name = jQuery(this).data('nama')

    $.ajax({
        url: '/tryout/pembahasan',
        method: 'post',
        data: { id, num },
        dataType: 'json',
        success: function (result) {
            if (result.success == false) {
                ...
                jQuery("#pembahasanTryout").removeClass('d-none')

                jQuery("#titleTryoutPembahasan").html(name)
                ...
                function showTest() {
                    ...
                    result.forEach((currentQuestion, questionNumber) => {
                        ...
                        output.push(`
                            <div class="question-tryout-pembahasan">
                                <div class="row box-white border-warning" id="questionPembahasanTryout">
                                ...
                                <div class="row mb-2 answer-test d-flex flex-row justify-content-between" id="answerPembahasanTryout">
                                    ${choices.join('')}
                                </div>
                                <div class="row mt-2 box-white border-warning" id="pembahasanPart">
                        ...
                        if (result[questionNumber].userAnswer == -1) {
                            jQuery("#boxNumberTryPembahasan").append(`
                ...
                function backPage() {
                    ...
                    jQuery("#boxNumberTryPembahasan").empty()
                    jQuery(".question-tryout-pembahasan").empty()
                    jQuery("#pembahasanTryout").addClass('d-none')
                }
                ...
                const containerQuiz = document.getElementById("tryoutPembahasanPage")

                showTest()

                const previousButton = document.getElementById("previousTryoutPembahasan")
                const nextButton = document.getElementById("nextTryoutPembahasan")
                const backPembahasan = document.getElementById("backTryoutPembahasan")
                const btnFinished = document.getElementById('btnFinish')
                const slides = document.querySelectorAll(".question-tryout-pembahasan")
                var questionCount = 0
